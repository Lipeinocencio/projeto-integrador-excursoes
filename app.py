...
# Mudamos o nome do banco para forçar o Render a criar um novo do zero
DB_NAME = 'sistema.db'
...
def inicializar_banco():
    ...
    # Novas Tabelas do CMS (Separadas e Organizadas)
    cursor.execute('''
...
# --- ROTAS DO CMS (Adicionar/Remover Dinâmico) ---
...
def comprar(id_viagem):
    if 'usuario_id' not in session: return redirect('/login')
    conn = sqlite3.connect(DB_NAME)
    conn.cursor().execute("INSERT INTO reservas (id_usuario, id_viagem) VALUES (?, ?)", (session['usuario_id'], id_viagem))
    conn.commit()
    conn.close()
